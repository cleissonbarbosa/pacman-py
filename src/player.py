...
class Pacman:
    ...
    def draw(self, surface, current_time):
        ...
        if mouth_open:
            ...
            start_angle, end_angle = angles.get(self.direction, (0.2 * math.pi, 1.8 * math.pi))
            pygame.draw.arc(surface, YELLOW, (self.x - self.radius, self.y - self.radius, self.radius * 2, self.radius * 2), start_angle, end_angle, 20)
        else:
